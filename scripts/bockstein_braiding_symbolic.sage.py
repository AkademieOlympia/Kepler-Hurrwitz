...
# This file was *autogenerated* from the file scripts/bockstein_braiding_symbolic.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1en10 = RealNumber('1e-10'); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_1en12 = RealNumber('1e-12'); _sage_const_4 = Integer(4); _sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_5 = Integer(5)
...
def phase_mod_N(phase, n, tol=_sage_const_1en10 ):
    """
...
def commutator_phase(n, flux=_sage_const_1 , tol=_sage_const_1en12 ):
    """
...
def compare_commutator_vs_w_n(n, flux=_sage_const_1 , tol=_sage_const_1en12 ):
    """
...
def torus_staircase_path(n):
    ...
    for _step in range(n):
        ...
        p1 = (Integer((a + _sage_const_1 ) % n), Integer(b % n))
        p2 = (Integer((a + _sage_const_1 ) % n), Integer((b + _sage_const_1 ) % n))
        path_yx.append((p0, p1, p2))
        a, b = a + _sage_const_1 , b + _sage_const_1 
    a, b = _sage_const_0 , _sage_const_0 
    for _step in range(n):
        p0 = (Integer(a % n), Integer(b % n))
        p1 = (Integer(a % n), Integer((b + _sage_const_1 ) % n))
        p2 = (Integer((a + _sage_const_1 ) % n), Integer((b + _sage_const_1 ) % n))
        path_xy.append((p0, p1, p2))
        a, b = a + _sage_const_1 , b + _sage_const_1 
    vertices = [(Integer(m % n), Integer(m % n)) for m in range(n)]
    edges = [(vertices[m], vertices[(m + _sage_const_1 ) % n]) for m in range(n)]
...
def verify_quantization(n, flux=_sage_const_1 , tol=_sage_const_1en12 ):
    """
    ...
    if phase is not None:
        phase_power_n_is_one = abs(phase**n - _sage_const_1 ) < tol
    if central:
...
def demo_compare_commutator_vs_w_n(n=_sage_const_4 , flux=_sage_const_1 , tol=_sage_const_1en12 ):
...
def demo_staircase_n4():
    ...
    n = _sage_const_4 
    a_spec = lambda P, Q: Integer((P + _sage_const_2  * Q) % n)
...
def print_staircase_demo(demo):
    ...
    path = demo["path"]
    print(f"YX-Treppe (3 Plaquettes): {path['path_YX'][:_sage_const_2 ]} ...")
    print(f"XY-Treppe (3 Plaquettes): {path['path_XY'][:_sage_const_2 ]} ...")
...
def main():
    ns = (_sage_const_2 , _sage_const_3 , _sage_const_4 , _sage_const_5 )
    print("=== [B] Demo 1/3: verify_quantization — phase(W_N)^N = 1 (nicht Operator-Claim W^N=I) ===")
    results = [verify_quantization(n, flux=_sage_const_1 ) for n in ns]
    print_verify_results(results)
    all_ok = all(r["quantization_ok"] for r in results)
    print(f"Alle Quantization-Checks: {all_ok}")
    print()
    cmp_demo = demo_compare_commutator_vs_w_n(n=_sage_const_4 , flux=_sage_const_1 )
