# Literaturliste (mit arXiv-Links)

Diese Liste dient als referenzierbare Basis fuer EABC/Hurwitz/Kepler-bezogene Arbeitspakete.
Sie ist thematisch gruppiert und kann direkt fuer Doku, Notizen und Paper-Entwuerfe genutzt werden.

## A. Hurwitz-Primes und Metakommutation

1. Cohn, H.; Kumar, A.  
   **Metacommutation of Hurwitz primes**  
   arXiv:1307.0443  
   Link: <https://arxiv.org/abs/1307.0443>

2. Forsyth, T.; Gurev, D.; Shrima, S.  
   **Metacommutation as a Group Action on the Projective Line over F_p**  
   arXiv:1503.06259  
   Link: <https://arxiv.org/abs/1503.06259>

## B. Quaternionische Arithmetik / Lipschitz-Hurwitz-Geometrie

3. Machiavelo, A.; Rocadas, D.  
   **Some Connections Between The Arithmetic and The Geometry of Lipschitz Integers**  
   arXiv:1201.5817 (HTML version)  
   Link: <https://arxiv.org/html/1201.5817v3>

4. (aktuelle Quaternionen-Faktorisierungsrichtung)  
   **The Dedekind-Hasse Criterion in Quaternion Algebras**  
   arXiv:2506.22651  
   Link: <https://arxiv.org/pdf/2506.22651>

## C. Octonionen / Octavian Integers

5. Knill, O.  
   **Goldbach for Gaussian, Hurwitz, Octavian and Eisenstein primes**  
   arXiv:1606.05958  
   Link: <https://arxiv.org/abs/1606.05958>

6. Grigorian, S.  
   **Octonionic structure operator and its right spectrum**  
   arXiv:2606.08299  
   Link: <https://arxiv.org/abs/2606.08299>

## D. Additive Primfaktor-Funktionen und Verteilungen

7. Mangerel, A.  
   **On the Distribution of Integers with Restricted Prime Factors I**  
   arXiv:1511.08038  
   Link: <https://arxiv.org/abs/1511.08038>

8. Coons, M.; Dahmen, S.  
   **On the residue class distribution of the number of prime divisors of an integer**  
   arXiv:0906.1029  
   Link: <https://arxiv.org/abs/0906.1029>

## E. Kubilius-/Erdos-Kac-nahe Modelle

9. Ford, K. (u.a., aktuelle Version)  
   **Poisson approximation of prime divisors of shifted primes**  
   arXiv:2408.03803  
   Link: <https://arxiv.org/html/2408.03803v4>

10. Yip, M.  
    **Multivariate and quantitative Erdos-Kac laws for Beatty sequences**  
    arXiv:2602.04875  
    Link: <https://arxiv.org/abs/2602.04875>

## F. Geometrische Primdarstellungen / Spektrale Perspektiven

11. Kolossvary, I.  
    **The Prime Grid. Introducing a geometric representation of natural numbers**  
    arXiv:1711.02903  
    Link: <https://arxiv.org/abs/1711.02903>

12. (aktuelle Spektralperspektive)  
    **Spectral Geometry of the Primes**  
    arXiv:2604.03351  
    Link: <https://arxiv.org/abs/2604.03351>

## G. Kepler / Kegelschnitt-Hintergrund

13. Givental, A.  
    **Kepler's laws and conic sections**  
    Link: <https://sumizdat.com/homepage/kepler_091615.pdf>

14. Montgomery, R.  
    **The Kepler Cone, Maclaurin Duality and Jacobi-Maupertuis metrics**  
    arXiv:2312.08173  
    Link: <https://arxiv.org/abs/2312.08173>

## I. L4 / Holonomie / Bockstein-Braiding

15. Hsin, P.-S.; Chen, Y.-A.  
    **Bockstein braiding statistics**  
    arXiv:2607.02280  
    Link: <https://arxiv.org/abs/2607.02280>  
    Kurzform: `[HsinChen2026]`  
    Interner Evidenzlayer: `docs/bockstein_braiding_layer.md` ([A]/[B]/[C])

## H. Interne PDFs (Mathematische Texte)

Quelle: `Desktop/Mathematische Texte`

16. **collatz_quantum_paper.pdf**  
    Lokal: `Desktop/Mathematische Texte/collatz_quantum_paper.pdf`

17. **eabc_spektrum_artikel.pdf**  
    Lokal: `Desktop/Mathematische Texte/eabc_spektrum_artikel.pdf`

18. **SM_Parameter_und_BM_Status.pdf**  
    Lokal: `Desktop/Mathematische Texte/SM_Parameter_und_BM_Status.pdf`

19. **BM_Arbeitsformeln_Uebersicht.pdf**  
    Lokal: `Desktop/Mathematische Texte/BM_Arbeitsformeln_Uebersicht.pdf`

20. **bm_toy_modell_langfassung.pdf**  
    Lokal: `Desktop/Mathematische Texte/bm_toy_modell_langfassung.pdf`

21. **bm_toy_modell_memo.pdf**  
    Lokal: `Desktop/Mathematische Texte/bm_toy_modell_memo.pdf`

22. **Aharonov-Bohm phase .pdf**  
    Lokal: `Desktop/Mathematische Texte/Aharonov-Bohm phase .pdf`

---

## Zitierhinweis fuer dieses Projekt

Fuer interne Referenzen kann folgende Kurzform genutzt werden:
- `[CohnKumar2013]` fuer arXiv:1307.0443
- `[ForsythGurevShrima2015]` fuer arXiv:1503.06259
- `[Knill2016]` fuer arXiv:1606.05958
- `[Mangerel2015]` fuer arXiv:1511.08038
- `[CoonsDahmen2009]` fuer arXiv:0906.1029
- `[Ford2024]` fuer arXiv:2408.03803
- `[Yip2026]` fuer arXiv:2602.04875
- `[Kolossvary2017]` fuer arXiv:1711.02903
- `[HsinChen2026]` fuer arXiv:2607.02280

Empfehlung: bei Paper-Entwuerfen spaeter auf BibTeX umstellen.
