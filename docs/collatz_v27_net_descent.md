...
```
PR #12  kanonisch  c0765e4…  Worktree h7mod256
PR #13  post-freeze — V28/Mod8/Nu2Bounds an PR #12 angeglichen (Kopie)
        ChannelSeven-Attack-Stack bleibt primär auf PR #12 / h7mod256
```

Vor Merge / Rebase / Freeze:

1. **Diff-Sanity:** `CollatzProofAttemptV28.lean` / `CollatzNetDescentMod8.lean` gegen PR #12/`h7mod256` prüfen.
2. **Kanonischer Abgleich:** Parametrisierungen \(243m+118\) / \(243m+209\) (Margins \(13m+5\) / \(13m+10\)) bleiben erhalten.
3. **Checkout-Verifikation:** Lean-Build `CollatzProofAttemptV28` ohne unerwartete Regression.
4. **Nicht zurückschreiben:** eine schlankere post-freeze-Fassung darf die PR-#12-Kette nicht überschreiben.
