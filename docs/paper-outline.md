...
### 4.3 Dumas-Schicht: holographische Auslassung (Musketiere-Spur)

**Status (#Energiedoku):** Die Dumas-Schicht formalisiert die Host-Auslassung als informationsaequivalente Komplementstruktur: Fuer einen Primvierling bestimmt die fehlende Komponente im Host-Tripel eindeutig die Host-Zuweisung. Die Vierkanalstruktur wird nicht durch zusaetzliche Wahlparameter erzeugt, sondern durch eine kanonische Komplementbeziehung zwischen Einzelhost und Dreiertraeger stabilisiert.

**Prägnant:** *Die Lücke im Tripel ist kein Verlust, sondern die Codierung des Hosts.*

**Lean (paper-zitierbar):**
...
- `Dumas_one_for_all_all_for_one` / `dumasLemma` — Zustandsbundle *Un pour tous, tous pour un*

**Evidence Chain E-046 → E-048 → E-032 → E-026** (getrennt von Collatz-V2 und Energiedoku E-033–E-044):

| Ebene | ID | Funktion | Status |
|---|---|---|---|
| Kanal / Primvierling | **E-046** | Komplementaritaet — `hostTriple` als Gap-Menge auf `primvierlingFinset` | `[A-T]` bewiesen |
| Dumas-Konsolidierung | **E-048** | Host-Komplementaritaet / holographische Auslassung; `dumas_gap_encodes_host` | `[A-T]` bewiesen |
| Ikosaeder-Bruecke | **E-032** | `LabelIntertwiningGraphAuto` → `IsEquivalentToCanonical` | `[A-T]` in Arbeit (1 `sorry`) |
| Zielhypothese | **E-026** | `MusketiereNeighborTripleHypothesis` — Zustandsbundle | `[C]` offen |

**Balance (kombinatorisch, nicht metaphorisch):** `|hostTriple| = |otherChannels| = 3` — vier Kanaele, eine Auslassung, drei getragene Komponenten (`dumasLemma_otherChannels_card`).

Dossier: `docs/dumas_lemma.md` · Lean: `KeplerHurwitz/PrimvierlingSymmetry.lean`

### 4.4 Collatz-V2 — lokale Evidence Chain (getrennt)

**Abgrenzung:** Collatz-V2 ist **lokale arithmetische Evidenz**, unabhaengig von Musketiere/Dumas und Energiedoku-Floquet.

| Schritt | Statement | Status |
|---|---|---|
| Good-Branch | `CollatzAttemptV2Mod4EqOneShrink` | `[A]` geschlossen |
| Tiefenextraktion | `n % 4 = 3 ⇒ ∃ d, ExactTwoAdicDepthOfSucc(d,n)` | `[A]` geschlossen (`CollatzProofAttemptV25`) |
| Bad-Run | `BadRunDepthExtractionStatement` | `[A]` geschlossen |
| Lokaler Shrink | `BadBranchEventuallyLocalShrinkStatement` | `[A]` geschlossen |
| **Offen** | `CollatzMod4ThreeGlobalDescentStatement`, globale Termination | offen |

Dossier: `docs/collatz_v2_evidence_chain.md` · Build: `lake build KeplerHurwitz.CollatzProofAttemptV25`

### 4.5 Fano-QEC-Brücke: CSS-Projektion und Steane-Abgrenzung (E-038, `[B]`)
...
### 4.6 Signierte Shell-Syndrome (E-041 `[B]` — Strukturclaim)
...
### 4.7 Symmetrieinvarianz (E-042 `[C]` — Invarianzvalidator)
...
### 4.8 Proposition S8 `[B]`: Signierte Fano-Shell-Trennung
...
### 4.9 Keil A-pre: Coupled Shell Resonance Graph (E-043-pre, `[C]`)
...
### 4.10 [[5,1,3]]-Stabilisator-Brücke (E-044, `[C]` — Manuskript Kapitel VII)
...
### 4.11 E-045-pre: Signiertes `[[5,1,3]]`-Stabilizer-Profil (nach Kapitel VII)
