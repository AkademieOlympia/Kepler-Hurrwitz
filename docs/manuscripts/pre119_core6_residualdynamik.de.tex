...
\subsection{Programm-Wand (PR-Stack)}

\begin{center}
\small
\begin{tabular}{@{}p{2.8cm}p{1.6cm}p{7.4cm}@{}}
\toprule
Schicht & Tag & Inhalt \\
\midrule
PR\,\#15--\#16
  & \tagA-Pfad
  & Core6-Existenz, Faseräquivalenz $C_e\simeq\N$, relative Dichte $1/8$ \\
PR\,\#17--\#18
  & \CA
  & Ein-Block-D2b, Pfad-AP (D3.2), Residual $\Leftrightarrow$ Reachability \\
PR\,\#19 (D3.4)
  & \CA
  & Dichotomie $\Residual=\FiniteExit\,\dot\cup\,\Forever$ (Draft) \\
Re-Entry / Reaches
  & \tagC
  & Konvergenz auf $\FiniteExit$; Ausschluss von $\Forever$ \\
\bottomrule
\end{tabular}
\end{center}

Die vollständige \emph{Reduktion} des Collatz-Engpasses ist damit formal
gefasst (\CA); die dynamische Lösung der beiden Residualklassen bleibt
unkonditioniert Forschungsfront (\tagC).
...
\begin{definition}[Endlicher Blockaustritt]
Lean-Bezeichner: \texttt{finiteBlockExitSet}.\\
$\FiniteExit(e_0)$ umfasst Starts mit expandierendem Core6-Aufenthalt, deren
nächster voller Block den Core6-Zylinder verlässt.
Tiefe~$0$ entspricht dem Ein-Block-Off-Core6-Anteil; Zeuge: $31\in\FiniteExit(1)$.
\end{definition}

\begin{definition}[Expandierende Grenzbahnen]
Lean-Bezeichner: \texttt{foreverExpandingBoundarySet}.\\
$\Forever(e_0)$ umfasst Starts, deren jeder expandierende Aufenthalt beim
nächsten Block wieder in expandierender Masse landet.
\end{definition}

\begin{theorem}[Residualfeinstruktur, D3.4 \CA]
...
\[
  ...
  \Forever(e_0).
\]
Die beiden Summanden sind disjunkt; der Ein-Block-Off-Core6-Anteil liegt in
$\FiniteExit(e_0)\subseteq\Residual(e_0)$, und ebenso
...
\begin{enumerate}[leftmargin=*]
  ...
  \item D3.4 lebt auf PR\,\#19
  (\texttt{cursor/core6-dynamic-d34-residual-2ed1}).
  \item Governance-Sequenz für PR\,\#19:
  CI-grün (Lean Action, Quality Gate, Evidence Audit)
  $\to$ Review
  $\to$ Merge auf akzeptierte D3.3-Basis (Stack \#15--\#18, kein Vorgriff auf
  \texttt{main})
  $\to$ \textbf{separater} Promotion-Commit $\CA\to\tagA$.
...
Residual-Erreichbarkeit
  & offen \tagC & Re-Entry / ForeverExp-Ausschluss \\
Collatz-Vermutung
