...
2. **`IdealUnivalence` (Univalenz-Schnittstelle):** `IdealUnivalenceHypothesis` und `IdealUnivalenceAxiom` modellieren das Voevodsky-Bild ($I_L \simeq I_R \implies I_L = I_R$) als optionales Postulat. Strukturelle Chiralität (E-068) blockiert den univalenten Kollaps und wird als topologische Obstruktion im höheren Identitätstyp sichtbar — **kein Lean-Theorem**.
...
4. **`period_equiv_zmod12` (Fundamentalperiode):** **Hypothesenfeld** — gewählte Modellierung der Fundamentalperiode als `≃ ZMod 12`. Das ist **nicht** ein Beweis von π₁ ≃ Z/12Z; die π₁-Interpretation ist separates Interface (`EabcMod12Pi1Hypothesis`). Endlicher Kanalcheck: `EabcMod12ChannelMapping` (E-072).
...
* **Nicht behauptet:** HoTT beweist nicht EABC-Isotropierestauration, Dumas (E-048) oder Dedekind-PID (E-053). `IdealPathsAsDH_QuatCells` und `Mod12ToIdealChiralityBridge` sind offen.
