---
title: Physikalische Referenz-Analogien zu EABC
date: 2026-07-05
status: "[C] methodische Physik-Analogie"
claim_boundary: >-
  Keine Identifikation von EABC mit QED, QHE oder Supraleitung. Die drei
  Referenzphänomene dienen ausschließlich als interpretative Resonanzanker
  für Orbit-/Kanal-/Defekt-Sprache — nicht als Beweis oder physikalische
  Vorhersage. Dumas-Orbit-Protokoll und Physik-Analogien bleiben strikt getrennt.
evidence_id: E-076
not_claimed:
...
> **No claim is made that EABC is QED, quantum Hall physics, or superconductivity.**  
> The three reference phenomena supply resonance anchors for orbit, channel, and defect language — not formal proof.

# Physikalische Referenz-Analogien zu EABC

**Stand:** 5. Juli 2026  
**Schicht:** L4 / Phase-C-Brücke  
**Didaktische Referenz:** [`meissner_effect.md`](../physics/meissner_effect.md) (reiner Meissner-Kontext, ohne EABC)  
**Bewertungsmemo:** [`meissner_analogy_assessment.md`](../theory/meissner_analogy_assessment.md) — Hilft Meissner zum Durchbruch? `[C]` Urteil (E-076)

---

## 1. Governance (verbindlich)

\[
\boxed{
\text{Physik-Analogie = Interpretation, nicht Beweis.}
}
\]
...
|---|---|---|
| EABC Formal Core | [`eabc_renormalisierungsprogramm.md`](../energiedoku_exports/eabc_renormalisierungsprogramm.md) | Retraktion, $24I_3$, Shell-Stapel `[A]`/`[B]` |
| Dumas Cone–Orbit | [`dumas_cone_orbit_model.md`](../theory/dumas_cone_orbit_model.md) | Normalform, H1–H11 `[A-T]`/`[B]` |
| Dumas-Orbit Protokoll | [`dumas_orbit_experimental_protocol.md`](dumas_orbit_experimental_protocol.md) | Empirie H12–H15, Nullmodelle — **ohne Physik-Brücke** |
| Parameter-Atlas | [`distilled_parameters.md`](../theory/distilled_parameters.md) | Diagnostik `[B]`/`[C]` |
| Arithmetisches Vakuum | [`arithmetic_vacuum_eabc_analogy.md`](../theory/arithmetic_vacuum_eabc_analogy.md) | Geschwister-`[C]`-Brücke (E-074) |

---

## 2. Drei Referenzachsen
...
## 3. Trennung: Dumas-Orbit vs. Physik-Analogien

$$\text{Dumas-Orbit-Protokoll (empirisch)} \;\neq\; \text{Physik-Referenz-Analogien (interpretativ)}$$
...
## 8. Vergleichstabelle AB / Klitzing / Meissner
...
**Das ist der belastbare Kern `[A]`/`[B]`** — unabhängig von jeder Physik-Analogie.
...
### Formaler Kern (nicht Analogie)

| Objekt | Definition | Status |
|---|---|---|
| $M_{\mathrm{geom}}$ | $24I_3$ — isotroper Referenztensor | `[A]`/`[B]` |
| $M_{\mathrm{eff}}(K^+)$ | $24I_3 + w_p vv^\top$, $w_p \in \{1,5,7,11\}$ | `[A]`/`[B]` |
| $\Delta(M)$ | $\lambda_{\max}(M) - \lambda_{\min}(M) = w_p$ (normiertes $v$) | `[B]` Messzweig |
| $R^*$ | Projektive EABC-Retraktion | `[A]`/`[B]` |

### Analogische Lesekette (explizit `[C]`)
...
## 11. Collatz / Dumas — vorsichtiger Verweis

Collatz V2.7 und Dumas Cone–Orbit teilen **methodische** Parallelen (Orbit, Abstieg, Normalform), **keine** physikalische Deduktion.
...
**Nicht erlaubt:** „Meissner-Exklusion erklärt H13“ oder „QHE beweist mod-$12$-Plateaus empirisch“.

---

## 12. Lean-Skizze: DefectShellModel / MeissnerLikeDefectExclusion

**Status:** Dokumentations-Skizze — **nicht** im Formal Core eingecheckt. Kein Evidenz-Upgrade.

```lean
/-
  DefectShellModel — Meissner-Analogie für EABC-Shell-Stapel.
  Governance: [C] interpretative Skizze; prime_norm_full_restoration bleibt [A].
-/

import KeplerHurwitz.Core  -- Platzhalter

/-- Bulk–Shell-Zerlegung eines effektiven Defekttensors. -/
structure DefectShellModel where
  bulkTensor    : Matrix (Fin 3) (Fin 3) ℝ
  shellDefect   : Matrix (Fin 3) (Fin 3) ℝ
  shellIndex    : ℕ
  anisotropy    : ℝ  -- Δ(M) = λ_max - λ_min

/-- Isotroper Referenzzustand (Meissner-Bulk-Analog). -/
def M_geom : Matrix (Fin 3) (Fin 3) ℝ := (24 : ℝ) • (1 : Matrix (Fin 3) (Fin 3) ℝ)

/-- Innere Anisotropie verschwindet ↔ effektiver Bulk ist isotrop. -/
def interiorAnisotropyVanishes (M : DefectShellModel) : Prop :=
  M.anisotropy ≤ ε  -- ε > 0 Toleranz; operationalisiert in Diagnostik [B]

/-- Meissner-ähnliche Defekt-Exklusion: Retraktion liefert 24I₃ im Bulk. -/
def MeissnerLikeDefectExclusion (K : EabcDefectConfig) (Rstar : EabcDefectConfig → EabcDefectConfig) : Prop :=
  interiorAnisotropyVanishes (defectShellOf K) ∧
  M_eff (Rstar K) = M_geom

-- Ziel-Invariante (bereits im Kern, unabhängig von Meissner-Metapher):
-- theorem prime_norm_full_restoration ...  -- [A]
-- theorem all_shells_tensor_restored ...    -- [A]
```
...
## 13. Mögliche experimentelle Diagnostik — Metriken `[B]`

Vorgeschlagene Metriken zur **messbaren** Annäherung an die Meissner-Analogie. Erst `[B]`, wenn in `diagnostics.py` / Export reproduzierbar.

| Metrik | Symbol / Definition | Quelle | Tag |
|---|---|---|---|
| **Innere Anisotropie** | $\Delta_{\mathrm{innen}} = \lambda_{\max} - \lambda_{\min}$ auf Bulk-Anteil | Renorm §5; `norm_signature_defect` | `[B]` |
| **Shell-Konzentration** | $\rho_{\mathrm{shell}} = \|\text{shellDefect}\|_F / \|M_{\mathrm{eff}}\|_F$ | Neu; Shell/Bulk-Split | `[B]` vorgeschlagen |
...
## 14. Governance-Tabelle (vollständig)
...
## 16. Abgrenzung Begriffe — erlaubt / vorsichtig / nicht verwenden
...
| Dokument | Rolle |
|---|---|
| [`meissner_effect.md`](../physics/meissner_effect.md) | Didaktische Meissner-Referenz (extern) |
| [`eabc_renormalisierungsprogramm.md`](../energiedoku_exports/eabc_renormalisierungsprogramm.md) | Formaler $24I_3$-Kern |
| [`dumas_orbit_experimental_protocol.md`](dumas_orbit_experimental_protocol.md) | Empirie — getrennt von Physik |
| [`distilled_parameters.md`](../theory/distilled_parameters.md) | Diagnostik-API |
| [`arithmetic_vacuum_eabc_analogy.md`](../theory/arithmetic_vacuum_eabc_analogy.md) | Geschwister-`[C]` (E-074) |
